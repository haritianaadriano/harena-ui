    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">Visualisez, filtrez vos dépenses et analysez-les intelligemment.</p>
        </div>
        <button
          onClick={() => setShowAddModal(true)}
          className="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-2.5 px-4 rounded-xl text-xs flex items-center gap-1.5 cursor-pointer shadow-lg shadow-emerald-500/10 transition-all border border-emerald-500/30 self-start md:self-auto"
        >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <div>
            ...
            <select
              ...
              className="px-2.5 py-1.5 w-full border border-slate-200 rounded-xl text-xs bg-slate-50/50 text-slate-700 focus:outline-none focus:ring-1 focus:ring-emerald-500"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-xs lg:col-span-2 space-y-4">
          <h3 className="text-sm font-bold text-slate-500 uppercase tracking-widest pl-1">Mouvements bancaires</h3>
          ...
          {loading ? (
            ...
            <div className="py-12 text-center space-y-3">
              <div className="h-12 w-12 bg-slate-50 text-slate-400 border border-slate-100 rounded-full flex items-center justify-center mx-auto">
                <Filter className="h-6 w-6" />
              </div>
              <h4 className="font-semibold text-xs text-slate-500">Aucune transaction trouvée</h4>
              <p className="text-[11px] text-slate-400 max-w-xs mx-auto">Essayez d'ajuster vos filtres de recherche ou sélectionnez un autre portefeuille.</p>
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="font-semibold text-slate-800 text-sm group-hover:text-slate-900 transition-colors">{tx.description}</h4>
                    ...
                    <div className="text-right space-y-1">
                      ...
                      <div className="flex items-center gap-1.5 justify-end">
                        ...
                        <span className="text-[9px] text-slate-400 font-bold uppercase font-mono">{tx.type}</span>
                      </div>
                    </div>
                  </div>
                );
        ...
        <div className="lg:col-span-1">
          {selectedTx ? (
            <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-xs space-y-6 sticky top-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-y-3 text-xs leading-relaxed">
                  ...
                  <div>
                    ...
                    <span className="font-semibold text-slate-700">{selectedTx.wallet.name}</span>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <span className="font-semibold text-slate-700 uppercase">{selectedTx.type}</span>
                  </div>
                  <div>
                    <span className="text-slate-400 block">Source / Tiers</span>
                    <span className="font-semibold text-slate-700">{selectedTx.source || 'Harena Core'}</span>
                  </div>
                  <div>
              ...
              {/* AI Analysis Pane */}
              <div className="bg-emerald-500/5 p-5 rounded-2xl border border-emerald-500/15 space-y-4">
                  ...
                  <div className="space-y-4 text-xs">
                    ...
                    <div className="space-y-1">
                      <div className="flex justify-between font-semibold text-slate-700">
                        ...
                        <span className={`font-bold ${analysis.anomaly_score > 0.6 ? 'text-red-500' : analysis.anomaly_score > 0.3 ? 'text-amber-500' : 'text-emerald-600'}`}>
                          {Math.round(analysis.anomaly_score * 100)}%
                        </span>
                      </div>
                      <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
                        <div 
                          className={`h-full ${analysis.anomaly_score > 0.6 ? 'bg-red-500' : analysis.anomaly_score > 0.3 ? 'bg-amber-500' : 'bg-emerald-500'}`}
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-100 text-[10px]">
                      <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-100 text-center">
                        <span className="text-slate-400 block uppercase tracking-wider mb-0.5">Sentiment</span>
                      ...
                      <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-100 text-center">
                        <span className="text-slate-400 block uppercase tracking-wider mb-0.5">Catégorie suggérée</span>
                        <span className="font-bold text-emerald-600 truncate block">{analysis.predicted_category}</span>
                      </div>
                    </div>

                  </div>
                ) : (
                  <div className="text-center py-4 space-y-1 bg-slate-50/50 rounded-xl border border-dashed border-slate-200">
                    <HelpCircle className="h-5 w-5 text-emerald-600/50 mx-auto" />
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-xs text-center py-12 space-y-4">
              <Info className="h-10 w-10 text-slate-400 mx-auto" />
              <div className="space-y-1">
                <h3 className="font-bold text-slate-800 text-sm">Aucune transaction sélectionnée</h3>
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-xl border border-slate-100 relative overflow-hidden">
            ...
            <form onSubmit={handleCreateTransaction} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1">Type d'opération</label>
                  <select
                  ...
                  >
                    ...
                    <option value="TRANSFER">Transfert d'épargne</option>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  >
                    <option value="COMPLETED">Complété (Réglé)</option>
                    <option value="PENDING">En attente (Débit futur)</option>
                  </select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1">Bénéficiaire / Tiers (Optionnel)</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1 font-mono">Référence (Optionnel)</label>
                  <input
                    type="text"
